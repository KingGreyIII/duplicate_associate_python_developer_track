...
colours = ["Red", "Yellow", "Dark Red"]

# Zip them together
fruit_colours = zip(fruits, colours)

# Iterate with enumerate to get the rank
for rank, pair in enumerate(fruit_colours, start=1):
    fruit, colour = pair  # <-- unpack the zip pair
    line = " -> ".join([fruit, colour])  # join the 2 strings
    print(f"Rank {rank}: {line}")

d = {"name": "Alice", "age": 30, "class": "Grade 9", "college": "St Louis High School"}
header = ",".join(list(d.keys())[:1])         # "name,age"
values = ",".join(map(str, d.values()))  # "Alice,30"
print(header)
print(type(header))
print(values)
